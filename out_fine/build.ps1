# libzplay CMake Build Script (Enhanced with Incremental Build)
# Usage:
...
function Test-BuildComplete($outputDir) {
    ...
    if (-not (Test-Path $objDir)) { return $false }
    if (-not (Test-Path $dbgDir)) { return $false }
...
Write-Host ""
Write-Host ("=" * 60)
Write-Host "  libzplay Enhanced Build System v2.1"
Write-Host ("=" * 60)
...
foreach ($tcName in $Toolchains.Keys) {
    ...
    $compParts = $tcName -split "-"
    if ($compParts.Count -ge 2 -and $compParts[1] -match '^(x64|x86|arm64)$') {
        $compShort = $compParts[0]
    } elseif ($compParts.Count -ge 2) {
        $compShort = "$($compParts[0])-$($compParts[1])"
    } else {
        $compShort = $compParts[0]
    }
