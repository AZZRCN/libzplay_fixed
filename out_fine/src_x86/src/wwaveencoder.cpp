...
const wchar_t *g_wave_encoder_error_strW[ENCODER_UNKNOWN_ERROR + 1] = {
			...
			TEncoderTellCallback tell_callback)
{
	...
	if(c_fRaw == 0)
	{
		...
		c_write_callback(const_cast<void*>((const void*)"RIFF"), 4, c_user_data); // 	Chunk ID
		c_write_callback(&nFileSize, 4, c_user_data); // chunk size = File Size - 8
		c_write_callback(const_cast<void*>((const void*)"WAVE"), 4, c_user_data); // File Format
		c_write_callback(const_cast<void*>((const void*)"fmt "), 4, c_user_data);	// SubChunk1 ID
		...
		c_write_callback(const_cast<void*>((const void*)"data"), 4, c_user_data); // SubChunk2 ID
