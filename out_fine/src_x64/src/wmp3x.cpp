...
const wchar_t *g_error_strW[WMP3X_UNKNOWN_ERROR + 1] = {
	L"No error.",
...
int __stdcall WMp3x::SetMasterVolume(unsigned int nLeftVolume,unsigned int nRightVolume)
{
	...
	MMRESULT res = waveOutSetVolume((HWAVEOUT)(UINT_PTR) wave_output_id, MAKELONG(l, r));
...
void __stdcall WMp3x::GetMasterVolume(unsigned int *pnLeftVolume,unsigned int *pnRightVolume)
{
	...
	DWORD volume;
	MMRESULT res = waveOutGetVolume((HWAVEOUT)(UINT_PTR) wave_output_id, &volume);
	if(res == MMSYSERR_NOERROR)
	{
...
int WMp3x::send_callback_message(WMp3x *instance, unsigned int message, UINT_PTR param1, UINT_PTR param2)
{
	instance->c_callback_ret = 0;
	
	if(instance->callback_func == 0)
		return 0;
	...
	else
	{
		...
		PostThreadMessage(instance->c_hThreadCallbackId, message + WM_USER, (WPARAM)param1, (LPARAM)param2);
	}
...
int  WMp3x::_SetFormat(TStreamFormat nFormat)
{
	...
	switch(nFormat)
	{
		...
		case sfWaveIn:
		{
			...
			if(c_decoder->Initialize((INT_PTR) &data, 0, 0, 0) == 0)
			{
				err(c_decoder->GetError());
				c_decoder->Release();
				c_decoder = 0;
...
// open save file
	...
	if(instance->c_save_filename)
	{
		...
		if(instance->c_encoder->Initialize(instance->c_isInfo->nSampleRate, instance->c_isInfo->nChannel,
			instance->c_isInfo->nBitPerSample, (UINT_PTR) hFile,
...
// =============================================================
		...
		{	
			...
			int return_value = send_callback_message(instance, MsgWaveBuffer, (UINT_PTR) instance->c_pchCurrentWaveBuffer, instance->c_nHaveBytesInWaveBuffer);
			// check return value
			if(return_value == 2)
			{
				LeaveCriticalSection(&instance->c_CS_waveOut);
...
int __stdcall WMp3x::SetFFTGraphParam(TFFTGraphParamID nParamID, intptr_t nValue)
{
	err(WMP3X_NO_ERROR);
...
intptr_t __stdcall WMp3x::GetFFTGraphParam(TFFTGraphParamID nParamID)
{
	err(WMP3X_NO_ERROR);
