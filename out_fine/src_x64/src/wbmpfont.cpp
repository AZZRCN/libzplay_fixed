...
WBmpFont::~WBmpFont()
{
	...
	if(_lpCharMap)
		free(const_cast<char*>(_lpCharMap));

	if(_lpText)
		free(const_cast<char*>(_lpText));
...
int WBmpFont::SetText(const char* lpText)
{
	if(!lpText)
		return 0;
	...
	_lpText = (const char*) malloc(_dwTextLen + 1);
	if(!_lpText) {
		_dwTextLen = 0;
		return 0;
	}

	strcpy((char*)_lpText, lpText);
...
int WBmpFont::Open(HBITMAP hbFont, const char* lpCharMap, int dwFontHeight)
{
...
// open new BmpFont
	...
	_lpCharMap = (const char*) malloc(_dwCharMapLen + 1);
	if(!_lpCharMap)
		return 0;

	strcpy((char*)_lpCharMap,lpCharMap);
...
int WBmpFont::GetTextWidth(const char *pText)
{
	int bitmap_width = 0;
