...
const wchar_t *g_recorder_error_strW[DECODER_UNKNOWN_ERROR + 1] = {
	L"WaveIn: No error.",
...
int SelectRecordingLine(UINT_PTR nMixerId, unsigned int nComponentID, char *user_mixer_line, unsigned int nLeftVolume, unsigned int nRightVolume);
...
int WWaveIn::Initialize(INT_PTR param1, INT_PTR param2, INT_PTR param3, INT_PTR param4)
{
...
int WWaveIn::OpenStream(WQueue *pQueue, int fDynamic, int param1, int param2)
{
	...
	if(SelectRecordingLine((UINT_PTR) c_hWaweIn, c_line_control_id, c_user_mixer_line, c_user_mixer_volume, c_user_mixer_volume) == 0)
	{
...
void CALLBACK WWaveIn::_WaveInProc(HWAVEIN hwi, UINT uMsg, DWORD_PTR dwInstance,DWORD_PTR dwParam1, DWORD_PTR dwParam2)
{
...
#pragma warning(pop)
	...
	if(mixerOpen(&hMixer, (UINT_PTR) nMixerId, 0, 0, MIXER_OBJECTF_HWAVEIN) != MMSYSERR_NOERROR)
		return 0;
