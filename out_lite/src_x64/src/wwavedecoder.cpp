...
const wchar_t *g_wave_error_strW[DECODER_UNKNOWN_ERROR + 1] = {
	L"No error.",
...
RIFF_CHUNK * get_riff_subchunk(char *data, unsigned int size, const char *id, unsigned int fSkipExtraCheck );
int get_riff_wave_id3_field(wchar_t **field, char *data, unsigned int size, const char *id);

wchar_t *get_riff_wave_id3_field(char *data, unsigned int size, const char *id);
...
int WWaveDecoder::Initialize(INT_PTR param1, INT_PTR param2, INT_PTR param3, INT_PTR param4)
{
...
//
	...
	if(fSkipExtraCheck == 0)
	{
		...
		while(ck <= upper_limit  && ( ck->size + RIFF_CHUNK_SIZE <= size))
		{
			...
			if(ck->id == nId)
			{
				if( (RIFF_CHUNK*) ((UINT_PTR) ck + ck->size) > upper_limit)
					return 0;

				return ck;
			}

			ck = (RIFF_CHUNK*) ((UINT_PTR) ck + ck->size + 8 + allign);
		}
	}
	else
	{
