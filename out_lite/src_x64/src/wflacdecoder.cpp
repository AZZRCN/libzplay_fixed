...
const wchar_t *g_flac_error_strW[DECODER_UNKNOWN_ERROR + 1] = {
	L"No error.",
...
FLAC_ID3_TABLE FLAC_OGG_ID3_ID[OGG_TABLE_SIZE] = {"ARTIST=", 7, ID3_INFO_ARTIST, 
...
int WFLACDecoder::Initialize(INT_PTR param1, INT_PTR param2, INT_PTR param3, INT_PTR param4)
{
...
// free previous fields
	...
	if(tags)
	{
		...
		for(i = 0; i < tags->data.vorbis_comment.num_comments; i++)
		{
			...
			{
				if(strnicmp((char*) tags->data.vorbis_comment.comments[i].entry, FLAC_OGG_ID3_ID[j].str_id, FLAC_OGG_ID3_ID[j].size) == 0)
				{
					unsigned int len = tags->data.vorbis_comment.comments[i].length - FLAC_OGG_ID3_ID[j].size;
					wchar_t *out = UTF8ToUTF16((char*) tags->data.vorbis_comment.comments[i].entry + FLAC_OGG_ID3_ID[j].size, len);
					if(out)
					{
						if(c_fields[FLAC_OGG_ID3_ID[j].id])
							free(c_fields[FLAC_OGG_ID3_ID[j].id]);
						c_fields[FLAC_OGG_ID3_ID[j].id] = out;
					}
